...
from navsim.agents.drsi.drsi_features import DRSIFeatureBuilder, DRSITargetBuilder
from navsim.agents.drsi.drsi_model import DRSIModel
from navsim.agents.drsi.drsi_callback import DRSICallback
...
def drsi_kd_imi_agent_loss_dropout(
        targets: Dict[str, torch.Tensor], predictions: Dict[str, torch.Tensor], config: DRSIConfig,
        vocab_pdm_score,
        regression_ep=False,
        three2two=True,
        include_dp=False
...
class DRSIAgent(AbstractAgent):
    def __init__(
        ...
        if self._config.version == 'default':
            self.model = DRSIModel(config)
        else:
            raise ValueError('Unsupported drsi version')
    ...
    def initialize(self) -> None:
        ...
        print('Loading full DRSI model', msg)
    ...
    def get_target_builders(self) -> List[AbstractTargetBuilder]:
        return [DRSITargetBuilder(config=self._config)]

    def get_feature_builders(self) -> List[AbstractFeatureBuilder]:
        return [DRSIFeatureBuilder(config=self._config)]
    ...
    def compute_loss(
        ...
        return drsi_kd_imi_agent_loss_dropout(targets, predictions, self._config, scores,
